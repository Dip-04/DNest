...
begin
  ...
  v_email := nullif(lower(trim(p_email::text)), '')::extensions.citext;
...
revoke all on function public.create_nest_invitation(uuid, extensions.citext) from public, anon;
...
grant execute on function public.create_nest_invitation(uuid, extensions.citext) to authenticated;
