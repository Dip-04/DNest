...
import { CalendarHeart, CircleGauge, Droplets, Sparkles, Sprout } from "lucide-react";
import { PeriodCalendar } from "@/components/period-calendar";
import { SexualHealthGuide } from "@/components/sexual-health-guide";
import { FormSubmitButton } from "@/components/form-submit-button";
import { saveTrackerSettings } from "@/features/period-tracker/actions";
import { getNestContext } from "@/lib/nest";
import { periodEnd, trackerModel, type PeriodCycle, type TrackerSettings } from "@/lib/period-tracker";
...
export default async function PeriodTrackerPage({ searchParams }: {
  ...
  return <div>
    ...
    <section className="period-summary mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><Summary icon={CircleGauge} label="Cycle day" value={model.currentCycleDay ? String(model.currentCycleDay) : "—"} /><Summary icon={Droplets} label="Next period" value={pretty(model.next?.start)} note={model.daysUntilPeriod != null && model.daysUntilPeriod >= 0 ? `${model.daysUntilPeriod} days away · estimated` : "Estimated"} /><Summary icon={Sparkles} label="Estimated ovulation" value={pretty(model.next?.ovulation)} /><Summary icon={CalendarHeart} label="Fertile window" value={model.next ? `${pretty(model.next.fertileStart)} – ${pretty(model.next.fertileEnd)}` : "Not available"} note="Estimated" /><Summary icon={Sprout} label="Fertility today" value={model.fertilityToday.label} note="Calendar estimate—not zero-risk contraception" /></section>
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-2">
      <section className="surface card"><h2 className="display text-3xl">Cycle history</h2>{cycles.length ? <div className="mt-4 grid gap-3">{cycles.map((cycle) => <div className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--border)] p-3" key={cycle.id}><div><strong>{pretty(cycle.start_date)} – {pretty(periodEnd(cycle, 5))}</strong><p className="muted text-xs">{cycle.period_length ?? 5}-day period{cycle.cycle_length ? ` · ${cycle.cycle_length}-day cycle` : ""}</p></div><span className="chip">Logged</span></div>)}</div> : <p className="muted mt-3">{viewingPartner ? "No shared cycles have been logged." : "Select a calendar date to log your first period."}</p>}</section>
      {canEditOwn && !viewingPartner && <form action={saveTrackerSettings} className="surface card grid gap-4"><h2 className="display text-3xl">Prediction & sharing</h2><p className="muted text-sm">Every period starts with an automatic five-day window. Cycle history improves future date estimates.</p><label className="label">Usual cycle length<input className="field" type="number" name="default_cycle_length" min="20" max="45" defaultValue={settings.default_cycle_length} required /></label><div className="label">Default period length<div className="field flex items-center justify-between"><span>5 days</span><span className="chip">Automatic</span></div></div><input type="hidden" name="default_period_length" value="5" /><label className="label">Your timezone<input className="field" name="timezone" defaultValue={safeTimeZone(settings.timezone)} required /></label><label className="flex items-start gap-3 rounded-2xl border border-[var(--border)] p-4 text-sm"><input className="mt-1 size-5 accent-[var(--rose)]" type="checkbox" name="share_with_partner" defaultChecked={Boolean(settings.share_with_partner)} /><span><strong>Share a read-only view with my partner</strong><small className="muted mt-1 block">They can see cycles and moods but cannot add, edit, or delete anything.</small></span></label><FormSubmitButton pendingLabel="Saving…">Save private settings</FormSubmitButton></form>}
    </div>
  </div>;
}
