...
export default async function PeriodTrackerPage({ searchParams }: {
  ...
  const partnerStored = visibleSettings?.find((item) => item.user_id === partner?.id && item.share_with_partner);
  const viewingPartner = params.owner === "partner" && Boolean(partnerStored && partner);
  ...
  return <div>
    ...
    {partnerStored && <nav className="mt-5 flex flex-wrap gap-2" aria-label="Tracker owner"><Link className={`btn ${!viewingPartner ? "btn-primary" : "btn-secondary"}`} href="/period-tracker">My tracker</Link><Link className={`btn ${viewingPartner ? "btn-primary" : "btn-secondary"}`} href="/period-tracker?owner=partner">{partner?.display_name}’s shared view</Link></nav>}
    ...
    <PeriodCalendar cycles={cycles} predictions={model.predictions} today={today} defaultPeriodLength={model.periodLength} readOnly={viewingPartner} moods={moods ?? []} />
