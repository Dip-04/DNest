...
export default async function VirtualEmotionsPage({ searchParams }: { searchParams: Promise<{ play?: string }> }) {
  ...
  return <AnimatedPage>
    ...
    {partner ? <VirtualEmotionsClient nestId={context.nest.id} userId={context.userId} myName={me?.display_name ?? "You"} partnerName={partner.display_name} initialEvents={events} initialPlay={initialPlay} /> : <section className="surface card mt-7"><h2 className="display text-3xl">Your second avatar is waiting</h2><p className="muted mt-2">Invite your partner to unlock shared emotions.</p></section>}
  </AnimatedPage>;
}
