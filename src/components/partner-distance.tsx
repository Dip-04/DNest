...
export function PartnerDistance({ initialMe, initialPartner }: {
  ...
  useEffect(() => {
    ...
    const update = () => setPartner((current) => ({ ...current,
      localTime: new Intl.DateTimeFormat("en", { timeZone: initialPartner.timezone, hour: "numeric", minute: "2-digit" }).format(),
    }));
    update();
  ...
  return <section className="between-us-widget surface card mt-5 overflow-hidden p-0">
    ...
    <p className="muted px-5 py-3 text-center text-xs">Precise live positions update while DNest is open.{me.accuracy != null && partner.accuracy != null ? ` GPS accuracy: you ±${Math.round(me.accuracy)} m · ${partner.name} ±${Math.round(partner.accuracy)} m.` : ""} Map © OpenStreetMap contributors.</p>
  </section>;
}
