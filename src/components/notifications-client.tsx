  ...
  return <>
    ...
    {items.length ? <div className="mt-7 grid gap-3">
      {items.map((item) => <article className={`notification-card surface card flex items-start gap-4 ${item.read_at ? "notification-read opacity-65" : "notification-unread"}`} key={item.id}>
        ...
        <div className="min-w-0 flex-1"><h2 className="font-bold">{item.title}</h2><p className="muted mt-1 text-sm">{item.body}</p><p className="muted mt-2 text-xs"><LocalDateTime value={item.created_at} timeStyle="short" /></p></div>
