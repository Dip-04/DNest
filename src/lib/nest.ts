...
export const getNestContext=cache(async():Promise<{userId:string;nest:NestContext}|null>=>{
 const session=await requireUser(); if(!session) return null;
 const {data:membership}=await session.supabase.from("nest_members").select("nest_id").eq("user_id",session.user.id).eq("status","active").maybeSingle();
