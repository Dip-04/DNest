...
export async function signUp(formData:FormData){ const parsed=signUpSchema.safeParse(Object.fromEntries(formData)); if(!parsed.success) redirect(toQuery("/sign-up",parsed.error.issues[0]?.message??"Invalid form")); const supabase=await createClient(); const origin=process.env.NEXT_PUBLIC_APP_URL??"http://localhost:3000"; const {error}=await supabase.auth.signUp({email:parsed.data.email,password:parsed.data.password,options:{data:{display_name:parsed.data.name},emailRedirectTo:`${origin}/auth/callback`}}); if(error) redirect(toQuery("/sign-up","We couldn’t create the account. Please try again.")); redirect(toQuery("/sign-in","Check your email to confirm your account, then sign in.")); }
